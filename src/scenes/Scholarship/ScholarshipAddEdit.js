...
const scholarshipFormConfigsPage1 = [
  {
    ...
    html: (model) => (
      <p className='text-muted'>
        {model.slug && `Slug: atila.ca/scholarship/${model.slug}`}
      </p>
    ),
  },
  {
    ...
    html: () => (
      <label htmlFor='description'>
        Short Description: Who is eligible for this scholarship? What should
        they do to apply? You will be able to give more details later.
      </label>
    ),
  },
  {
  ...
  },
  {
    keyName: "open_date",
    type: "date",
    isHidden: (scholarship) =>
      scholarship.metadata && !scholarship.metadata.not_open_yet,
    html: () => (
      <label htmlFor='open_date'>
        When does the scholarship open?{" "}
  ...
  },
  {
    ...
    html: () => (
      <label htmlFor='location'>
        Is the scholarship limited to certain locations?{" "}
...
class ScholarshipAddEdit extends React.Component {
  ...
  updateForm = (event) => {
    ...
    if (event.target.name === "location") {
      ...
      const scholarship = nestedFieldUpdate(
      ...
      if (
        Array.isArray(scholarship[event.target.name]) &&
        !Array.isArray(value)
      ) {
  ...
  submitForm = (event) => {
    ...
    const scholarship = ScholarshipsAPI.cleanScholarship(
      this.state.scholarship
    );
    this.setState({ scholarship });
    ...
    if (isAddScholarshipMode) {
      ...
      postResponsePromise = ScholarshipsAPI.put(
        scholarship.id,
        scholarship,
        locationData
      );
          ...
          <div className='card shadow p-3'>
            ...
            {!userProfile && (
              <h4>
                ...
                Warning, you must be logged in to add a scholarship
              </h4>
            )}
            ...
            <div className='my-2' style={{ clear: "both" }}>
              <button
                className='btn btn-outline-danger float-right col-md-4'
            ...
            <div className='my-2' style={{ clear: "both" }}>
              {pageNumber !== 2 && (
                <button
                  className='btn btn-outline-primary float-right col-md-6'
              ...
              {pageNumber !== 1 && (
                <button
                  className='btn btn-outline-primary float-left col-md-6'
                  onClick={() => this.changePage(pageNumber - 1)}
                >
                  Prev
                </button>
